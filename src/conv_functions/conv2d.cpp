...
ImageT dyn_conv2d(ImageT inp, KernelT weight, ConvOptions opt) {
    ...
    if (opt.padding.is_same || opt.padding.values[0] != 0 || opt.padding.values[1] != 0) {
        input = dyn_pad_input(inp, weight, opt);
    } else {
        input = inp;
    }
...
ImageT static_pad_input(ImageT input, KernelT weight, int* stride, int* dilation, int* padding, int padding_same) {
    ...
    if (padding_same) {
        ...
        new_input.height = input.height * stride[0] - stride[0] + dilation[0] * (weight.height - 1) + 1;
        new_input.width = input.width * stride[1] - stride[1] + dilation[1] * (weight.width - 1) + 1;
        pad_h = (new_input.height - input.height) / 2;
        pad_w = (new_input.width - input.width) / 2;
    } else {
        pad_h = padding[0];
        pad_w = padding[1];
        new_input.height = input.height + 2 * pad_h;
        new_input.width = input.width + 2 * pad_w;
    }
    new_input.data = conv::runtime::conv_malloc((int)sizeof(int)*new_input.width*new_input.height*new_input.batch_size*new_input.in_channels);
    dyn_var<int> new_idx;
    dyn_var<int> old_idx;
...
ImageT static_conv2d(ImageT inp, KernelT weight, static_var<int*> stride, static_var<int*> dilation, static_var<int*> padding, static_var<int> padding_same) {
    ...
    // pad the input
    ImageT input;
    if (padding_same || padding[0] != 0 || padding[1] != 0) {
        input = static_pad_input(inp, weight, stride, dilation, padding, padding_same);
    } else {
        input = inp;
    }

    ImageT output;
    output.height = (input.height - dilation[0] * (weight.height - 1) - 1) / stride[0] + 1;
    output.width = (input.width - dilation[1] * (weight.width - 1) - 1) / stride[1] + 1;
    output.in_channels = weight.out_channels;
    output.batch_size = input.batch_size;
    dyn_var<int> size = output.width * output.height * output.batch_size * output.in_channels;
    output.data = conv::runtime::conv_calloc(size, (int)sizeof(int));
            ...
            for (dyn_var<int> in_ch = 0; in_ch < input.in_channels; in_ch = in_ch + 1) {
                ...
                for (dyn_var<int> h = 0; h < output.height; h = h + 1) {
                    for (dyn_var<int> w = 0; w < output.width; w = w + 1) {
                        ...
                        for (dyn_var<int> i = 0; i < weight.height; i = i + 1){
                            for (dyn_var<int> j = 0; j < weight.width; j = j + 1) {
                                in_idx = bid * input.in_channels * input.width * input.height + in_ch * input.width * input.height + (h * stride[0] + i * dilation[0]) * input.width 
                                            + (w * stride[1] + j * dilation[1]);
                                weight_idx = out_ch * weight.in_channels * weight.width * weight.height + in_ch * weight.width * weight.height + i * weight.width + j;
                                output.data[out_idx] = output.data[out_idx] + input.data[in_idx] * weight.data[weight_idx];
                            }
                        }
